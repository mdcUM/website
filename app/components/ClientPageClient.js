...
// ===== helpers & small UI pieces (client-safe) =====
...
export default function ClientPageClient({
  ...
  // Allow passing a pre-fetched client OR look it up here from CLIENTS
  const slugify = (s) => s.toLowerCase().replace(/\s+/g, "-");
  const client =
    clientProp ||
    CLIENTS.find((c) => slugify(c.name) === clientId);
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 py-20">
        <div className="text-center">
          ...
          <p className="text-white/70 mb-6">
            The requested client {clientId} does not exist.
          </p>
          <Link
            href="/clients"
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {client.tags?.map((tag) => <TagPill key={tag} text={tag} />)}
            </div>
            ...
            <Section title="What We Did" className="mb-6">
              {client.whatwedid ? (
                <div className="space-y-6">
                  <p className="text-base text-white/90 leading-relaxed">{client.whatwedid}</p>
            ...
            >
              <Section title="Client Feedback">
                <blockquote className="rounded-xl border border-white/10 bg-white/[0.05] p-4">
                  ...
                  <footer className="text-xs text-white/60">— {client.testimonial.author}</footer>
                </blockquote>
              </Section>
            </motion.div>
          )}
          ...
          {client.results && (
            ...
            >
              <Section title="Results">
                <p className="text-sm text-white/85 leading-relaxed">{client.results}</p>
              </Section>
            </motion.div>
          )}
        </div>
